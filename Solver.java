...
public class Solver implements Runnable {
    ...
    public static void test(String pathName) throws FileNotFoundException {
        ...
        ArrayList<Chromosome> bestFits = new ArrayList<>();
        ...
        for (; startSeed < endSeed; ++startSeed) {
            ...
            for (int j = 0; j < nThreads; ++j) {
                ...
                for (int k = 0; k < chrs.size(); ++k) {
                    Chromosome tmp = (Chromosome) chrs.get(k);
            ...
            while (on) {
                for (Solver solver : solvers) {
                    if (solver.hasSolution()) {
                        solutionSeeds.addChromosome(startSeed);
                        Chromosome solution = ((Chromosome) solver.getBestFit());
                        System.out.println("solution found");
                        System.out.println("soludion =\n" + solution.printGrid());
                        bestFits.addChromosome(new Pair<>(solution, solution.getFitness()));
                ...
                if (on) {
                    ...
                    if (allHaveEnded) {
                        noSolutionSeeds.addChromosome(startSeed);
                        bestFits.addChromosome(new Pair<>((Chromosome) bestFit, bestFit.getFitness()));
                        on = false;
                        break;
                    }
                }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        //test("/home/ricky/NetBeansProjects/Sudoku2/src/sdk/settings_test.txt");
    }
}
